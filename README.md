# BLE COMPILER - ESP32-S3 リモート開発システム

**iPad/スマホからBluetooth経由でESP32にファームウェアを書き込めるシステムです。**

🌐 **WebApp:** https://ble-compiler.netlify.app/

BLE経由で以下の機能を提供します：

- ✅ **ファームウェア書き込み（BLE OTA）** - USBケーブル不要
- ✅ **Wi-Fi設定（プロビジョニング）** - BLE経由でWi-Fi接続
- ✅ **リアルタイムデバッグ** - BLE経由でシリアルモニタ表示

**注意: ファームウェア更新は完全にBLE経由で実行されます。Wi-FiはHTTP OTAには使用しません。**
...
ESP32-S3に初回だけファームウェアを書き込みます（2回目以降はBLE経由で更新可能）。
...
詳細は [MiconSide/README.md](MiconSide/README.md) を参照してください。

### 2️⃣ WebAppを開く

iPad/スマホで以下のURLを開きます（Bluefyアプリ推奨）：

**https://ble-compiler.netlify.app/**

### 3️⃣ BLE接続

1. WebAppの **[Connect Device]** ボタンをクリック
2. デバイスリストから **ESP32-S3-MICON** を選択
3. 接続完了（ステータスが「接続済み」に変わります）

### 4️⃣ Wi-Fi設定（初回のみ）

Wi-Fi機能を使う場合のみ設定してください（BLE機能のみなら不要）：

1. **Wi-Fi Provisioning** パネルでSSIDとパスワードを入力
2. **[Send]** をクリック
3. デバイスが自動的に再起動してWi-Fiに接続

### 5️⃣ ファームウェア更新（BLE OTA）

1. **Firmware Upload** パネルで `.bin` ファイルを選択
...
1. **Debug Monitor** パネルの **[Subscribe]** をクリック
2. ESP32からのログがリアルタイムで表示されます
3. 1秒ごとに「Hello World via BLE」が送信されます（サンプル動作）

---

## 📋 目次

1. [クイックスタート（使い方）](#-クイックスタート使い方)
2. [プロジェクト構成](#プロジェクト構成)
3. [システムアーキテクチャ](#システムアーキテクチャ)
4. [BLE通信仕様](#ble通信仕様)
5. [WiFi・OTA通信仕様](#wifiota通信仕様)
6. [BLE OTAの仕組み（初心者向け解説）](#ble-otaの仕組み初心者向け解説)
7. [マイコン側（ESP32）の使い方](#マイコン側esp32の使い方)
8. [WebApp側（iPad/Bluefy）の使い方](#webapp側ipaddbluefy-の使い方)
9. [動作フロー](#動作フロー)
...
#### 方法 1: デプロイ済みWebApp（推奨）

以下のURLを開くだけで使用できます：

**🌐 https://ble-compiler.netlify.app/**

- Bluefyアプリ（iPad）で開いてください
- ChromeやEdge（Android/PC）でも動作します
- Safariは Web Bluetooth API 非対応のため使用不可
...
- [Connect Device] ボタン → デバイス選択ダイアログ
- 接続完了後、[Disconnect] ボタンに切り替わる
...
2. パスワードを入力
3. [Send] をクリック
4. デバイスが Wi-Fi に接続されると、IPアドレスが表示される（参考情報）

⚠️ **注意:** Wi-Fi接続はプロビジョニング時の確認用です。OTA通信はBLE経由で実施されます。
...
1. [Choose File] で `.bin` ファイルを選択（最大2MB）
2. [Upload Firmware] ボタンをクリック
...
- [Subscribe] → ログ受信開始（ESP32からのログがリアルタイムで表示）
- [Clear] → ログバッファをクリア
- コマンド欄にコマンドを入力して [Send]

**デフォルト動作:**

- ESP32は1秒ごとに「Hello World via BLE」を自動送信（サンプル動作）
